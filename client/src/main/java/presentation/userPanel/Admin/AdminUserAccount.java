...
//	AdminBlService_Stub service = new AdminBlService_Stub();
	...
	String[] tebleH = {"员工编号","姓名","职务","性别","年龄","城市","所属机构"};
	...
	private void initTable(){
		...
		model = new TableModel(tableV, tebleH,isCellEditable);
...
//        tcm.getColumn(7).setCellEditor(new EditorDelete(table.getRowCount()));
	...
	private void showSearchResult(){
		...
		model = new TableModel(getVector(searchUsers),tebleH,isCellEditable);
...
//			int row = table.rowAtPoint(p);
//			int column = table.columnAtPoint(p);
//			System.out.println("Mouse entered :   row"+row+"   column"+column);
